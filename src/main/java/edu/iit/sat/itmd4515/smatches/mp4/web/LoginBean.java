...
public class LoginBean extends AbstractJSFBean {
    ...
    public boolean isFan() {
        return facesContext.getExternalContext().isUserInRole("fan");
    }

    public boolean isDj() {
        return facesContext.getExternalContext().isUserInRole("dj");
    }    
    ...
    public String getPortalPathByRole(String path) {
        if (isAdmin()) {
            ...
            return "/stationPortal" + path + FACES_REDIRECT;
        } else if (isFan()) {
            return "/listenerPortal" + path + FACES_REDIRECT;
        } else {
